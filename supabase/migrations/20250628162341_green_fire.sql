...
1. Yeni Tablolar
  - `invite_codes`
    - `id` (uuid, primary key)
    - `invite_code` (text, unique, not null) - Davet kodu
    - `inviter_id` (uuid, not null) - Davet eden kullanıcı
    - `inviter_name` (text, not null) - Davet eden adı
    - `inviter_email` (text, not null) - Davet eden email
    - `invited_email` (text, not null) - Davet edilen email
    - `is_used` (boolean, default false) - Kullanıldı mı
    - `used_by_id` (uuid) - Kullanan kullanıcı ID
    - `used_by_email` (text) - Kullanan email
    - `used_at` (timestamptz) - Kullanım tarihi
    - `expires_at` (timestamptz, not null) - Son kullanma tarihi
    - `invite_type` (text, default 'user') - Davet tipi
    - `can_invite_others` (boolean, default true) - Başkalarını davet edebilir mi
    - `created_at` (timestamptz, default now())
    - `is_active` (boolean, default true)

2. Güvenlik
  - RLS etkin
  - Kimlik doğrulaması yapanlar kendi davetlerini görebilir
  - Davet kodları herkese açık (validasyon için)

3. İndeksler
  - Davet kodu için unique index
  - Davet eden kullanıcı için index
  - Aktif kodlar için filtered index
*/
